...
/** Library-wide or in-book full-text search with exact passage locators. */
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    Text(
                        if (scopeBookUuid == null) "Search library" else "Search in book",
        ...
        Column(modifier = Modifier.fillMaxSize().padding(padding)) {
            ...
            LazyColumn {
                if (state.bookMatches.isNotEmpty()) {
                    item { SearchSectionTitle("Books") }
                    items(state.bookMatches, key = { "book-${it.uuid}" }) { book ->
                        Column(
                            modifier = Modifier
                                .fillMaxWidth()
                ...
                if (state.passageMatches.isNotEmpty()) {
                    item { SearchSectionTitle("Passages") }
                    items(
                        state.passageMatches,
                        key = { "passage-${it.bookUuid}-${it.chapterIndex}-${it.charOffset}" },
                    ) { passage ->
                        Column(
                            modifier = Modifier
                                .fillMaxWidth()
                                .clickable {
                                    onOpenPassage(passage.bookUuid, passage.chapterIndex, passage.charOffset)
                                }
                                .padding(horizontal = 20.dp, vertical = 10.dp),
                        ) {
                            Text(
                            ...
                            Text(
                                if (scopeBookUuid == null) {
                                    ...
                                    "Chapter ${passage.chapterIndex + 1}"
                                },
